<template>
  <view class="ei-calendar-item"
    ...
    <view class="ei-calendar-item__top" >
      <view  v-for="(item, index) in topList" :class="[item.class || 'ei-calendar-item__bottom__today',{'ei-calendar-item--single-selected': selected === 'single'}]"  :key="index">{{item.text}}</view>
    </view>
		<!--   class="ei-calendar-item__center"  背景样式 -->
    <view>
      <view  class="ei-calendar-item__center__day" v-if="centerList != []">{{day.day}}</view>
    </view>
    <view class="ei-calendar-item__bottom">
      <!-- <view v-if="day.isToday" class="ei-calendar-item__bottom__today">今天</view> -->
			<!-- 把今天去电 -->
			<view :style="{textAlign:'center'}"  :class="[item.class,{'ei-calendar-item--single-selected': selected === 'single'}]" v-for="(item,index) in bottomList" :key="index">
				{{item.text}}
			</view>
    </view>
  </view>
...
export default {
  ...
  computed: {
    ...
    customDate() {
			// console.log(this.day.customDate)
      return this.day.customDate || {};
    },
    topList() {
			// console.log(this);
			// console.log( this.customDate.top );
