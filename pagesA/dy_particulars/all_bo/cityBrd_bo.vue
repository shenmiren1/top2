<template>
	<view>
		...
		<view class="All_session_time">
			...
			<view>{{ time | time_com }} {{ time | time_zhou }}</view>
		</view>
		...
		<view class="button">
			<view class="buttonH">
				...
				<view class="buttonHone" v-if="index == 0">{{res.city}}影院明细</view>
				<view class="buttonHone" v-if="index == 1">{{res.brd}}影院明细</view>
			...
			<view class="buttonB" v-for="(it, ine) in itemlist" :key="ine">
				<view class="buttonB_Headerson">
					<view class="buttonB_Headerson_name" v-if="index == 0">{{ it.cinema_name }}</view>
					<view class="buttonB_Headerson_name" v-if="index == 1">{{ it.cinema_name }}</view>
...
export default {
	...
	methods: {
		time_click(i) {
			...
			var { id, mid, index ,city,brd} = this.res;
			var date = [];
			var url = this.baseURL;
			if (i == 1) {
				th_time = moment(th_time)
					.subtract(-1, 'days')
					.format('YYYY-MM-DD');
			} else {
				th_time = moment(th_time)
		...
		async bd_index(url, All_data) {
			...
			data.forEach((item, index) => {
				let feneDate = res.fene[item['cinema_code']];
				// 票房占比
