<template>
	<view>
		...
		<view class="header_tab">
			<view @tap="header_tab_click(i)" :class="[ header_tab_index == i ? 'active' : '']" v-for="(item,i) in header_tab_data"
			 :key="i">
				{{item}}
			</view>
		</view>
	...
	export default {
		...
		onReachBottom() {
			...
			if (this.res.Br) {
				...
				var res = this.res;
				let movie_id = res.movie_id;
				var url = 'https://db.topcdb.com/topprogram/get-report?page=' + page  + '&movie_id=' + movie_id ;
				page++
				...
				let city = res.city;
				var url = 'https://db.topcdb.com/topprogram/get-report?page=' + page + '&city=' + city + '&district=' + district +
					'&brd=' + brd;
				page++
				this.page_index = page;
				this.db_index(url, 1)
		...
		methods: {
			...
			tz_click(id, brd, district, city) {
				...
				var url =  './list/list?id=' + id + '&brd=' + brd + '&district=' + district + '&city=' + city
				
				console.log(url);
				uni.navigateTo({
					url: '../list?id=' + id + '&brd=' + brd + '&district=' + district + '&city=' + city
				})
			},
			// 发送请求
			db_index(url, index) {
