...
<template>
	<view class="ynGalleryPanel">
		<view  class="gallerypanel" style="filter:brightness(1);border-radius:1upx" :style="{'background':bkstyle,'height':galleryheight+'px','width':'100%'}">
			<!-- scroll-with-animation="true" scroll-with-animation :scroll-into-view="toview" -->
			<scroll-view class="scroll-view"  scroll-x="true"  scroll-with-animation  :scroll-left="scrollLeft" @scroll="scroll">
	...
	export default {
		...
		methods: {
			...
			clickimg(idx, imgviewobj) {
				...
				this.$emit('clickimg_one',imgviewobj)
			},
					...
					if (this.istouchmove == true) {
						...
						var TTT = setTimeout(function() {
							if (that.activeviewindex == ai) {
								that.$emit('clickimg_one',that.sviewlst.imgsview[that.activeviewindex]);
							}
						}, 300)
					}
