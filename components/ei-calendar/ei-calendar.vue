		...
		<view class="ei-calendar__container">
			<view class="ei-calendar__container__header">
				...
				<view class="ei-calendar__container__header__date">
					...
					<view class="ei-calendar__container__header__date__show">{{ currentDateShow }}</view>
					<view class="ei-calendar__container__header__date__btn" hover-class="is-hover" :hover-start-time="20" :hover-stay-time="70" @click="toYear(1)">
						<view class="ei ei-d-right"></view>
					</view>
					<view class="ei-calendar__container__header__date__edge today" hover-class="is-hover" :hover-start-time="20" :hover-stay-time="70" @click="toYear(0)">
						<!-- 今天 -->
					</view>
				</view>
				<view class="ei-calendar__container__header__week">
					<view class="ei-calendar__container__header__week__day" v-for="(day, index) in week" :key="index">{{ day }}</view>
				</view>
			</view>
			<swiper
				class="ei-calendar__container__content"
				vertical
				:current="currentIndex"
				:duration="duration"
				@animationfinish="animationfinish_tap"
			...
			>
				<swiper-item class="ei-calendar__container__content__item" v-for="(item, itemIndex) in showMonthList" :key="itemIndex">
					...
					<view v-for="(row, index1) in item.days" :key="index1" class="ei-calendar__container__content__item__row">
...
export default {
	...
	methods: {
      ...
      if (Array.isArray(this.customDate)) {
		...
		onDayClick(day, isDisabled = undefined, virtual = undefined) {
		...
		toYear(year) {
			...
			if (year === 0) {
				...
				var data = moment(data).subtract(year, 'years').format('YYYY-MM-DD');
				var arr = data.split('-')
