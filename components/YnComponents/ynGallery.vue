...
<template>
	<view class="ynGalleryPanel">
		<view  class="gallerypanel" style="filter:brightness(1);border-radius:1upx" :style="{'background':bkstyle,'height':galleryheight+'px','width':'100%'}">
			<scroll-view class="scroll-view" scroll-x="true" scroll-with-animation="true" :scroll-into-view="toview" :scroll-left="scrollLeft" @scroll="scroll">
				<!-- scroll-view-panle 容纳左右空块+中间img块 -->
				<view class="scroll-view-panle" @touchend='touchEndHandle' @touchstart='touchStartHandle' @touchmove='touchMoveHandle'>
					<!-- 左侧留空 必须用min-width属性 用width属性无效-->
					<view class="leftview" :style="{'min-width':sviewlst.leftview.w+'px','height':galleryheight+'px'}"></view>
					<!-- 中间图像横向列表 ==-->
					<!-- 外层 高度与画廊高度一样 -->
					<view :id="'imgviewpanel'+index" :ref="'imgviewpanel'+index" :class="[activeviewindex == index? 'trans-fadeout':  '']"
					       @animationstart="animationstartHandle" @animationend="animationendHandle" 
						   v-for="(item,index) in sviewlst.imgsview" :key="index" 
						...
						<ynTriangleBadge  v-if="showbadge && badegtype=='trian'" :width="badegwidth" :height="badegheight" :text="item.badeg" :bgcolor="item.badegcolor"
					...
					<view class="rightview" :style="{'min-width':sviewlst.rightview.w+'px','height':galleryheight+'px'}"></view>
				</view>
			</scroll-view>
...
<script>
	import ynTriangleBadge from '../ynTriangleBadge/ynTriangleBadge.vue'
	export default {
		components: {
			ynTriangleBadge,
		},
		...
		methods: {
			...
			scroll: function(e) {
				...
				// this.scoll_s(e.mp.detail.scrollLeft)
				var that = this;
				// console.log(e)		
				that.newleft =  e.mp.detail.scrollLeft;
				that.oldleft.scrollleft = that.newleft;
				//scroll左边点+屏幕中点 总是随着拖动而变化
				let cenpos =e.mp.detail.scrollLeft + that.screencenterpos;
				// console.log('中锚点位置:'+cenpos); 
				for (let i in that.sviewlst.imgsview) {
					let item = that.sviewlst.imgsview[i];
					if ((item.l <= cenpos) && (item.r >= cenpos)) {
						let imgcpos = item.l + parseInt(item.w / 2);
						let aoffset = cenpos - imgcpos;
						that.offset = aoffset;
						that.upDataScrollviewLayout(i);
						break;
					}
				}
			},
			scoll_s:function(scoll_left){
